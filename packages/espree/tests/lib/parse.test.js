...
const allPiecesJson = JSON.parse(
	fs.readFileSync("./tests/fixtures/parse/all-pieces.json", "utf8"),
);
...
describe("parse()", () => {
	...
	describe("general", () => {
		...
		it("should output the same value for range and start and end in templateElement with range", () => {
			...
			assert.strictEqual(
				ast.body[0].expression.quasis[0].start,
				ast.body[0].expression.quasis[0].range[0],
			);
			assert.strictEqual(
				ast.body[0].expression.quasis[0].end,
				ast.body[0].expression.quasis[0].range[1],
			);
			assert.strictEqual(
				ast.body[0].expression.quasis[1].start,
				ast.body[0].expression.quasis[1].range[0],
			);
			assert.strictEqual(
				ast.body[0].expression.quasis[1].end,
				ast.body[0].expression.quasis[1].range[1],
			);
		});
		...
		it("should output the same value for loc and start and end in templateElement with loc", () => {
			...
			assert.strictEqual(
				ast.body[0].expression.quasis[0].start,
				ast.body[0].expression.quasis[0].loc.start.column,
			);
			assert.strictEqual(
				ast.body[0].expression.quasis[0].end,
				ast.body[0].expression.quasis[0].loc.end.column,
			);
			assert.strictEqual(
				ast.body[0].expression.quasis[1].start,
				ast.body[0].expression.quasis[1].loc.start.column,
			);
			assert.strictEqual(
				ast.body[0].expression.quasis[1].end,
				ast.body[0].expression.quasis[1].loc.end.column,
			);
		});
		...
		it("should reset lastToken on each parse", () => {
			...
			assert.deepStrictEqual(
				[ast.loc.end.line, ast.loc.end.column],
				[1, 5],
			);
		});
		...
		it("should not mutate config", () => {
			espree.parse(
				"foo",
				Object.freeze({ ecmaFeatures: Object.freeze({}) }),
			);
		});
