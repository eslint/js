...
const allPiecesJson = JSON.parse(
	fs.readFileSync(`${__dirname}/../fixtures/parse/all-pieces.json`, "utf8"),
);
...
const scriptOnlyTestFiles = allTestFiles.filter(
	filename => !filename.includes("modules"),
);

const moduleTestFiles = allTestFiles.filter(
	filename =>
		!filename.includes("not-strict") && !filename.includes("edge-cases"),
);
...
describe("ecmaVersion", () => {
			...
			describe(filename, () => {
				...
				it("should parse correctly when sourceType is script", async () => {
					...
					const expected = await import(
						`${pathToFileURL(absolutePath).href}.result.js`
					);
		...
		moduleTestFiles.forEach(filename => {
			describe(filename, () => {
				...
				it("should parse correctly when sourceType is module", async () => {
					...
					try {
						expected = await import(
							`${pathToFileURL(absolutePath).href}.module-result.js`
						);
					} catch {
						expected = await import(
							`${pathToFileURL(absolutePath).href}.result.js`
	...
	describe("general", () => {
		...
		it("Should allow 'latest' as value", () => {
			const expected = espree.parse("let foo = bar;", {
				ecmaVersion: espree.latestEcmaVersion,
				sourceType: "module",
			});

			const actual = espree.parse("let foo = bar;", {
				ecmaVersion: "latest",
				sourceType: "module",
			});
