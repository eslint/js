...
describe("Tester", () => {
	describe("getRaw", () => {
		...
		it("should keep NaN", () => {
			assert.deepStrictEqual(getRaw({ value: NaN }), { value: NaN });
		});

		it("should keep Infinity", () => {
			assert.deepStrictEqual(getRaw({ value: Infinity }), {
				value: Infinity,
			});
		});
		...
		it("should keep bigint values", () => {
			assert.deepStrictEqual(getRaw({ value: BigInt("123") }), {
				value: BigInt("123"),
			});
		});
		...
		it("should keep RegExp objects", () => {
			...
			assert.deepStrictEqual(getRaw({ value: /foo/u }), {
				value: /foo/u,
			});
			...
			assert.deepStrictEqual(getRaw({ value: /\d\/\d/u }), {
				value: /\d\/\d/u,
			});
		});
	});
	...
	describe("getAstCode", () => {
		...
		it("should keep NaN", () => {
			assert.deepStrictEqual(test({ value: NaN }), { value: NaN });
		});

		it("should keep Infinity", () => {
			assert.deepStrictEqual(test({ value: Infinity }), {
				value: Infinity,
			});
		});
		...
		it("should keep bigint values", () => {
			assert.deepStrictEqual(test({ value: BigInt("123") }), {
				value: BigInt("123"),
			});
		});
		...
		it("should keep RegExp objects", () => {
			...
			assert.deepStrictEqual(test({ value: /foo/u }), { value: /foo/u });

			// check about escape sequences of `JSON.stringify`.
			assert.deepStrictEqual(test({ value: /"foo"/u }), {
				value: /"foo"/u,
			});
			assert.deepStrictEqual(test({ value: /\d\/\d/u }), {
				value: /\d\/\d/u,
			});
		});
	});
