...
describe("Class fields", () => {
	describe("class C { f = g }", () => {
		...
		it("The class-field-initializer scope's variableScope is itself.", () => {
			...
			assert.strictEqual(
				fieldInitializerScope.variableScope,
		...
		it("The class-field-initializer scope has only the reference 'g'.", () => {
			...
			assert.strictEqual(
				fieldInitializerScope.references[0].identifier.name,
				"g",
			);
		});
	...
	describe("class C { [fname] }", () => {
		...
		beforeEach(() => {
			const ast = espree.parse("class C { [fname] }", {
				ecmaVersion: 13,
			});
			const manager = analyze(ast, {
				ecmaVersion: 13,
		...
		it("The class scope has only the reference 'fname'.", () => {
			...
			assert.strictEqual(
				classScope.references[0].identifier.name,
				"fname",
			);
		});
	...
	describe("class C { [fname] = value }", () => {
		...
		beforeEach(() => {
			const ast = espree.parse("class C { [fname] = value }", {
				ecmaVersion: 13,
			});
			const manager = analyze(ast, {
				ecmaVersion: 13,
	...
	describe("class C { #f = g; e = this.#f }", () => {
		...
		beforeEach(() => {
			const ast = espree.parse("class C { #f = g; e = this.#f }", {
				ecmaVersion: 13,
			});
			const manager = analyze(ast, {
				ecmaVersion: 13,
				childVistorKeys: KEYS,
			});
