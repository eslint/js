...
describe("Class static blocks", () => {
	describe("class C { static { var a; let b; const c = 1; function d(){} class e {} } }", () => {
		...
		it("the class static block scope should have two child scopes, a function and a class scope", () => {
			...
			assert.strictEqual(
				classStaticBlockScope.childScopes[0].type,
				"function",
			);
			assert.strictEqual(
				classStaticBlockScope.childScopes[0].upper,
				classStaticBlockScope,
			);
			assert.strictEqual(
				classStaticBlockScope.childScopes[1].type,
				"class",
			);
			assert.strictEqual(
				classStaticBlockScope.childScopes[1].upper,
				classStaticBlockScope,
			);
		});
		...
		it("the class static block scope's `variableScope` is itself", () => {
			...
			assert.strictEqual(
				classStaticBlockScope.variableScope,
				classStaticBlockScope,
			);
		});
		...
		it("the class static block scope should be returned by ScopeManager#acquire for the `StaticBlock` node from the AST", () => {
			...
			assert.strictEqual(staticBlockNode.type, "StaticBlock");
			assert.strictEqual(
				scopeManager.acquire(staticBlockNode, /* inner = */ false),
				classStaticBlockScope,
			);
			assert.strictEqual(
				scopeManager.acquire(staticBlockNode, /* inner = */ true),
				classStaticBlockScope,
			);
		});
		...
		it("the `StaticBlock` node itself doesn't declare any variables", () => {
			...
			assert.strictEqual(staticBlockNode.type, "StaticBlock");
			assert.strictEqual(
				scopeManager.getDeclaredVariables(staticBlockNode).length,
				0,
			);
		});
		...
		it("the class static block scope should have 5 variables: `a`, `b`, `c`, `d`, `e`", () => {
			...
			assert.deepStrictEqual(
				classStaticBlockScope.variables.map(v => v.name),
				expectedVariableNames,
			);
			assert.deepStrictEqual(
				[...classStaticBlockScope.set.keys()],
				expectedVariableNames,
			);
			assert.deepStrictEqual(
				[...classStaticBlockScope.set.values()],
	...
	describe("class C { static { function f(){} f(); } }", () => {
		...
		it("the class static block scope should have one reference, to the variable `f`", () => {
			...
			assert.strictEqual(
				classStaticBlockScope.references[0].resolved,
				classStaticBlockScope.variables[0],
			);
		});
		...
		it("the variable `f` should have one reference, from the class static block scope", () => {
			...
			assert.strictEqual(
				f.references[0],
				classStaticBlockScope.references[0],
			);
		});
	});
	...
	describe("class C { static { C.x; } }", () => {
		...
		beforeEach(() => {
			const ast = espree.parse("class C { static { C.x; } }", {
				ecmaVersion: 13,
			});
			const scopeManager = analyze(ast, {
				ecmaVersion: 13,
	...
	describe("let a; class C { static { lbl: { this.b = a } } }", () => {
		...
		it("the class scope should have one `through` reference, a reference to `a`", () => {
			...
			assert.strictEqual(
				classScope.through[0].resolved,
				globalScope.set.get("a"),
			);
		});
		...
		it("the class static block scope should have one `through` reference, a reference to `a`", () => {
			...
			assert.strictEqual(classStaticBlockScope.through.length, 1);
			assert.strictEqual(
				classStaticBlockScope.through[0].resolved,
				globalScope.set.get("a"),
			);
		});
		...
		it("the block scope should have one reference, a reference to `a", () => {
			...
			assert.strictEqual(blockScope.references.length, 1);
			assert.strictEqual(
				blockScope.references[0].resolved,
				globalScope.set.get("a"),
			);
		});
	...
	describe("class C { static { a; } }", () => {
		...
		beforeEach(() => {
			const ast = espree.parse("class C { static { a; } }", {
				ecmaVersion: 13,
			});
			const scopeManager = analyze(ast, {
				ecmaVersion: 13,
	...
	describe("let a; class C { static { let a; a; } static { a; let a; } }", () => {
		...
		it("the class scope should have two child scopes, and those are two different class static block scopes", () => {
			...
			assert.strictEqual(
				classScope.childScopes[0].block.type,
				"StaticBlock",
			);
			assert.strictEqual(
				classScope.childScopes[1].type,
				"class-static-block",
			);
			assert.strictEqual(
				classScope.childScopes[1].block.type,
				"StaticBlock",
			);
			assert.ok(
				classScope.childScopes[0] !== classScope.childScopes[1],
				"Class static block scopes are same",
			);
			assert.ok(
				classScope.childScopes[0].block !==
					classScope.childScopes[1].block,
				"Class static block scopes are on the same node",
			);
			assert.ok(
				classScope.childScopes[0].upper ===
					classScope.childScopes[1].upper,
				"Class static block scopes don't have the same upper scope",
			);
		});
		...
		it("each of the two class static block scopes should have one variable `a` and one reference to that variable", () => {
			...
			classScope.childScopes.forEach(classStaticBlockScope => {
				...
				assert.ok(
					variable.scope === classStaticBlockScope,
					"Variable is from another scope",
				);
				...
				assert.ok(
					staticBlockNode.range[0] <= referenceIdentifier.range[0] &&
						referenceIdentifier.range[1] <=
							staticBlockNode.range[1],
